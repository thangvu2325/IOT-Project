...
const ScanQRScreen: FunctionComponent<ScanQRScreenProps> = () => {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      const data = new Uint8ClampedArray(imageBin.length);
      for (let i = 0; i < imageBin.length; i++) {
        data[i] = imageBin.charCodeAt(i);
      }
      const code = jsQR(data, result.assets[0].width, result.assets[0].height);
      if (code) {
        alert(`Mã QR vừa quét có dữ liệu: ${code.data}`);
      } else {
  ...
  const handleBarCodeScanned = ({ type, data }: BarcodeScanningResult) => {
    setScanned(true);
    alert(`Mã QR với dữ liệu ${data} đã được quét!`);
  };
  // Appbar
