...
export class CoapService {
                ...
                switch (obj.testId) {
                  case 'MAIN_MCU_BLE_SIGNAL':
                    if (obj.results.result === 'TEST_SUCCESSFUL') {
                      device = { ...obj.results.details };
                    }
                    break;
                  case 'Sensor':
                    if (obj.results.result === 'TEST_SUCCESSFUL') {
                      device.sensors = { ...obj.results.details };
                    }
                    break;
                  case 'BATTERY_VOLTAGE':
                    if (obj.results.result === 'TEST_SUCCESSFUL') {
                      device.battery = { ...obj.results.details };
                    }
                    break;
                  case 'CELLULAR_SIM':
                    if (obj.results.result === 'TEST_SUCCESSFUL') {
                      device.sim = { ...obj.results.details };
                    }
                    break;
                  case 'CELLULAR_SIGNAL':
                    if (obj.results.result === 'TEST_SUCCESSFUL') {
                      device.signal = { ...obj.results.details };
                    }
                    break;
                }
              });
  ...
  sendRequest() {
    ...
    this.server.listen(() => {
      ...
      req.end(JSON.stringify(this.jsonData));
    });
  }
}
